...
export const loader = async ({ params, request }) => {
  ...
  const metafield = responseJson.data.paymentCustomization?.metafield?.value &&
...
export const action = async ({ params, request }) => {
  ...
  // If the ID is `new`, then we're creating a new customization. 
  // Otherwise, we will use the update mutation.
  if (id === "new") {
    const response = await admin.graphql(
      `#graphql
        mutation createPaymentCustomization($input: PaymentCustomization!) {
    ...
    const response = await admin.graphql(
      `#graphql
        mutation updatePaymentCustomization($id: ID!, $input: PaymentCustomization!) {
...
export default function PaymentCustomization() {
  ...
  const [paymentMethodName, setPaymentMethodName] = useState(loaderData.paymentMethodName);
